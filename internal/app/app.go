...
func Init(dataDir string) error {
	...
	fmt.Printf("ShellGate initialized.\n\nData directory:\n  %s\n\nConfig:\n  %s\n\nMCP token:\n  %s\n\nSSH keys:\n  %s\n\nAudit logs:\n  %s\n\nEdit config.yaml and start ShellGate again.\n", dataDir, filepath.Join(dataDir, "config.yaml"), filepath.Join(dataDir, "mcp_token"), filepath.Join(dataDir, "keys"), filepath.Join(dataDir, "logs"))
	return nil
}
...
func Serve(dataDir string, handler func(*hosts.Registry, *broker.Broker, bool, string) http.Handler) error {
	r, e := config.Load(dataDir)
	if e != nil {
		return e
	}
	reg := hosts.New(r.Hosts)
	b := broker.New(reg, audit.New(r.AuditDir, r.Config.Audit.FilenameTemplate, r.Config.Audit.OutputLimit), r)
	defer b.Close()
	srv := &http.Server{
		Addr:              r.Config.MCP.Listen,
		Handler:           handler(reg, b, r.Config.MCP.Auth.Enabled, r.Token),
